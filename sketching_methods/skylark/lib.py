...
from skylark.errors import SkylarkError,LowerLayerError,UnsupportedError,InvalidObjectError,ParameterMistmatchError,DimensionMistmatchError
...
class NumpyAdapter:
  def __init__(self, A):
    if A.dtype.type is not numpy.float64:
      raise UnsupportedError("Only float64 matrices are supported.")
    if A.base is not None:
      raise UnsupportedError("Passing a numpy matrix view is not supported.")
  ...
  class DistMatrixAdapter:
    def __init__(self, A):
      if El.TagToType(A.tag) != ctypes.c_double:
        raise UnsupportedError("Only double precision matrices are supported.")
...
def adapt(obj):
    ...
    if isinstance(obj, numpy.ndarray):
        ...
        raise InvalidObjectError("Invalid/unsupported object passed as parameter")
